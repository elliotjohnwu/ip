		...
		try {
			if (line.toLowerCase().startsWith("todo")) {
				...
				if (text.length < 2 || text[1].isEmpty() || !text[0].equalsIgnoreCase("deadline")) {
					throw new JellyfishException("Invalid deadline format. Use: deadline <description> /by <time>");
				}
				...
				if (text.length < 2 || text[1].isEmpty() || !text[0].equalsIgnoreCase("event")) {
					throw new JellyfishException("Invalid event format. Use: event <description> /from <start> /to <end>");
				}
